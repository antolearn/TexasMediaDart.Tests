...
test.describe('Identity API - Registration', () => {
  ...
  test('should reject registration with duplicate email', async ({ request }) => {
    ...
    expect(
        isRegisterErrorResponse(registerResult.body)
    ).toBe(true);

    if (!isRegisterErrorResponse(registerResult.body)) {
        throw new Error(
        'Expected duplicate registration error response.'
        );
    }

    expect(registerResult.body.title)
        .toBe('Registration failed.');

    expect(registerResult.body.status)
        .toBe(409);
  ...
  test('should reject registration when passwords do not match', async ({ request }) => {
    ...
    expect(
        isRegisterValidationErrorResponse(registerResult.body)
    ).toBe(true);

    if (!isRegisterValidationErrorResponse(registerResult.body)) {
        throw new Error(
        'Expected registration validation error response.'
        );
    }

    expect(registerResult.body.title)
        .toBe('Registration validation failed.');
  ...
  test('should reject registration with invalid email format', async ({ request }) => {
    ...
    expect(registerResult.body.errors.Email)
        .toContain('A valid email address is required.');
});
