...
import { isRefreshTokenErrorResponse } from '../models/refresh-token.models';
  ...
  test(
    ...
    async ({ request }) => {
      ...
      if (!isRefreshTokenErrorResponse(refreshResult.body)) {
        throw new Error('Expected refresh token error response after logout.');
      }

      expect(refreshResult.body.title).toBe('Token refresh failed.');

      expect(refreshResult.body.status).toBe(401);

      expect(refreshResult.body.detail).toBe('Invalid or expired refresh token.');
    }
  );
});
